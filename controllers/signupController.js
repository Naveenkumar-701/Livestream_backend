...
export const registerUser = async (req, res) => {
  try {
    ...
    if (userExist && userExist.recrootUserType === "tempStudent") {
      ...
      user = await User.findByIdAndUpdate(
        userExist._id,
        {
          ...
          password: null, // Important: clear any old password
        },
        { new: true }
      );
    } else {
      ...
      user = await User.create({
        ...
        email_is_verified: false,
        referral_code,
        password: null, // Explicitly no password yet
      });
    }
    ...
    return res.status(200).json({
      ...
      User: user,
      token,
      referral_code, // optional, for debugging
      userID: user._id,
    });
  } catch (error) {
...
export const verifyOtp = async (req, res) => {
  try {
    ...
    if (String(user.referral_code) !== String(otp).trim()) {
      return res.status(400).json({ message: "Invalid OTP" });
    }

    // Mark as verified and clear OTP
    user.email_is_verified = true;
    // user.referral_code = null; // Clear OTP after use
    await user.save();
